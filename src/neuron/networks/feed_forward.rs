...
#[derive(Debug, Clone)]
pub struct StandardFeedForwardNetwork<L: Layer> {
    layers: Vec<L>,
}

impl<L: Layer> StandardFeedForwardNetwork<L> {
...
impl<L: Layer> Network<L> for StandardFeedForwardNetwork<L> {
    fn len(&self) -> usize {
        self.layers.len()
    }
...
impl<L: Layer> Regression<L> for StandardFeedForwardNetwork<L> {
...
mod tests {
    ...
    fn test_network_predict() {
        let l1 = StandardLayer::new(3, 2, Dense::new(), ReLu::new());
        let l2 = StandardLayer::new(4, 3, Dense::new(), Sigmoid::new());
        let l3 = StandardLayer::new(1, 4, Dense::new(), Linear::new());
